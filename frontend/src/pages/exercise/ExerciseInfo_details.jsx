...
export default function ExecrciseInfo_details({value}){

    return(
    <>
        <div className='flex justify-between my-2'>
                <h3 className='text-[#0077ff] font-bold'>DURATION</h3>
                <p>{value.duration?value.duration:"..."}</p>
            </div>

            <div className=' my-2'>
                <h3 className='text-[#0077ff] font-bold'>INSTRUCTIONS</h3>
                <ul className='list-disc list-inside'>
                {value.instructions?.map((res,idx)=>(
                        <li key={idx}>{res}</li>
                ))}
            ...
            <div className='exercise-common-mistakes my-2'>
                ...
                <ul className='list-disc list-inside '>
                    {value.commonMistakes?.map((res,idx)=>(
                        <li key={idx}>{res}</li>
                    ))}
            ...
            <div className='exercise-breathing-tips my-2'>
                ...
                <ul className='list-disc list-inside'>
                {value.breathingTips?.map((res,idx)=>(
                    <li key={idx}>{res}</li>
                ))}
